...
| `terminal.messageRenderLimit` | number | `0` | Keep only the newest N chat items in the TUI (`0` = unlimited). Older messages stay in the session; a notice marks the trimmed head. Enforced continuously while live messages are appended, not only on rebuild |
