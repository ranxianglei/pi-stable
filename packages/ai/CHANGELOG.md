...
- Fixed OpenAI-compatible requests to send an empty string instead of `null` as the `content` of assistant messages that contain only tool calls, which some proxies reject or respond to with a hung stream ([#1](https://github.com/ranxianglei/pi-stable/issues/1)).
- Fixed `npm run check` to validate generated model data before type checking, so a missing or stale `packages/ai/src/providers/data/` fails with one actionable error ("Run `npm run hydrate:model-data`") instead of ~116 opaque tsgo TS2345 errors from model-id literals collapsing to `never` ([#21](https://github.com/ranxianglei/pi-stable/issues/21)).
...
- The root entrypoint (`pi-stable-ai`) is now core-only and side-effect free. The old global API moved to the temporary `pi-stable-ai/compat` entrypoint, a strict superset of the root: switching a file's import path is the only migration step. Moved symbols include `stream`/`complete`/`streamSimple`/`completeSimple`, `getModel`/`getModels`/`getProviders` (now deprecated aliases of `getBuiltinModel`/`getBuiltinModels`/`getBuiltinProviders` from `pi-stable-ai/providers/all`), `registerApiProvider`/`unregisterApiProviders`/`resetApiProviders`/`getApiProvider`, `getEnvApiKey`/`findEnvKeys`, `setBedrockProviderModule`, the per-API lazy stream wrappers (`anthropicMessagesApi`, ...), and the image-generation API.
...
- API implementation modules moved from `src/providers/` to `pi-stable-ai/api/*`, renamed by API id (`anthropic` -> `api/anthropic-messages`, `google` -> `api/google-generative-ai`, `mistral` -> `api/mistral-conversations`, `amazon-bedrock` -> `api/bedrock-converse-stream`), each exporting exactly `stream` and `streamSimple`. The old per-impl export names (`streamAnthropic`, `streamSimpleAnthropic`, ...) and legacy raw API subpaths (`./anthropic`, `./google`, `./openai-completions`, ...) are gone; import raw API implementations through `pi-stable-ai/api/*`.
- Removed the `pi-stable-ai/base` selective-provider entrypoint; use the root/core APIs with explicit `createModels()` collections and provider factories for isolated bundles.
...
- To keep the old global API temporarily, change imports from `pi-stable-ai` to `pi-stable-ai/compat`. The compat entrypoint preserves `stream`/`complete`, generated catalog reads, API registry helpers, env API-key helpers, lazy API wrappers, and image globals, but it will be removed in a future release.
- To migrate to the new runtime, create a `Models` collection and call methods on it:

  ```ts
  import { builtinModels } from "pi-stable-ai/providers/all";
...
- For an isolated provider set, register provider factories explicitly:
  ...
  ```ts
  import { createModels } from "pi-stable-ai";
  import { anthropicProvider } from "pi-stable-ai/providers/anthropic";

  const models = createModels();
  models.setProvider(anthropicProvider());
  ```

- To call a raw API implementation directly, import from `pi-stable-ai/api/*` and pass a compatible model plus auth/options yourself. Raw API modules export `stream` and `streamSimple`; use `.result()` on the returned stream for `complete`/`completeSimple` behavior:

  ```ts
  import { streamSimple } from "pi-stable-ai/api/anthropic-messages";
  import { getBuiltinModel } from "pi-stable-ai/providers/all";
...
- One provider factory per built-in provider under `pi-stable-ai/providers/*` (e.g. `anthropicProvider()`, `openrouterProvider()`), plus `pi-stable-ai/providers/all` with `builtinProviders()`/`builtinModels()` and typed `getBuiltin*` catalog reads. Generated catalogs are split per provider, so importing one provider pulls one catalog; `sideEffects` metadata makes the package tree-shakeable.
...
- Added `pi-stable-ai/base` and direct provider registration exports for bundlers that want selective provider transports without root built-in registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
