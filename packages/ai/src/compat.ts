 ...
 *
 * Existing apps switch imports from "pi-stable-ai" to
 * "pi-stable-ai/compat" unchanged; new code uses `createModels()`
...
/** @deprecated Static catalog read. Use `getBuiltinModel` from "pi-stable-ai/providers/all" or `Models.getModel()`. */
export const getModel = getBuiltinModel;

/** @deprecated Static catalog read. Use `getBuiltinModels` from "pi-stable-ai/providers/all" or `Models.getModels()`. */
export const getModels = getBuiltinModels;

/** @deprecated Static catalog read. Use `getBuiltinProviders` from "pi-stable-ai/providers/all" or `Models.getProviders()`. */
...
function getBuiltinProviderForModel(model: Model<Api>) {
	...
	return provider &&
		(provider.apiSurface?.(model.api) || provider.getModels().some((candidate) => candidate.api === model.api))
		? provider
		: undefined;
}
