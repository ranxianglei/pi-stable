...
if [[ "$SKIP_DEPS" == "false" ]]; then
    ...
    npm install --include=optional --force --ignore-scripts \
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/pi-stable.exe"
    else
        echo "  $OUTPUT_DIR/$platform/pi-stable"
    fi
done
